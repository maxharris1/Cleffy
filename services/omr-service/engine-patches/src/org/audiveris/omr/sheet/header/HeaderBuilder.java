...
import org.audiveris.omr.sheet.time.PdfHeaderTimeHints;
...
public class HeaderBuilder
{
    ...
    public void processHeader ()
    {
        ...
        // Retrieve header key-sigs, stopping before a proven numerical time column
        final int keyWidth = PdfHeaderTimeHints.keySearchWidth(system, maxHeaderWidth);
        int keyOffset = keyColumn.retrieveKeys(keyWidth);
        setSystemKeyStop(keyOffset);

        // Retrieve header time-sigs
        int timeOffset = timeColumn.retrieveTime();
        if ((keyWidth < maxHeaderWidth) && (timeOffset < 0)) {
            logger.info(
                    "PDF header time hints not committed for S#{}; restoring original key search",
                    system.getId());
            restoreKeysToClefStop();
            keyOffset = keyColumn.retrieveKeys(maxHeaderWidth);
            setSystemKeyStop(keyOffset);
            timeOffset = timeColumn.retrieveTime();
        }
    ...
    //------------------------//
    /**
     * Discard provisional keys and header stops so the original key search
     * can run after a time-column reservation was not committed.
     */
    private void restoreKeysToClefStop ()
    {
        for (Staff staff : system.getStaves()) {
            if (staff.isTablature()) {
                continue;
            ...
            header.stop = (clefStop != null) ? clefStop : header.start;
        }
    }
