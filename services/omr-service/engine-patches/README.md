...
The host follow-up at `72bd29b` selected `cleffy-rsi-omr-20` while stamping
the report and artifact keys svc-21. Its unchanged official 7/16 table does
not validate this patch. Cycle 15 remains the single pending recognition
hypothesis; see its ledger for the byte-identical XML evidence. The official
PDF ingestion path now requires the selected container's built service
revision to match the checkout before any engine call or cache reuse, and
cache metadata must record that observed revision. Existing unverified
entries must be regenerated by the host using a matching svc-21 image.
The container name and the upstream `5.11.0` version alone are insufficient.
An engine mismatch is an environment error, so the host must not reuse the
previous report as the result of a failed attempt.
