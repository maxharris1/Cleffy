...
rejection. Svc-21 was reserved for the cycle-15 rest-template candidate, which
closed without acceptance; patch 0006 and its sources are removed here.
Svc-22 applies only the next ledger-fragment-dot hypothesis to accepted
svc-19 engine behavior.

## 0006 — rejected quarter-rest template experiment

Cycle 15 corroborated quarter-rest glyphs with same-page templates. The
supplied host table stayed at 7/16 and did not recover Air's printed rest.
The patch, vendored `SymbolsBuilder` / `QuarterRestTemplateMatcher`, Docker
compilation entries, and standalone rest-template controls are removed.
Commit `72bd29b` retains the rejected source. Svc-21 remains reserved.
See `docs/omr-rsi-cycle-15.md` for the provenance caveat and conservative
verdict. Provenance checks in `src/eval/candidate.ts` stay active.

## 0007 — reject leftover neighboring-ledger ink as an augmentation dot

A leftover fragment of a neighboring head's ledger can be admitted as an
augmentation dot when no concrete ledger was recognized at that ordinate.
`DotFactory` now rejects that mark before ordinary dot recording, and only
when `LedgerFragmentEvidence.isFragment` proves a thin horizontal source-ink
run through the neighboring head that protrudes on both sides. Detached
dots and staff-interior marks keep the historical path. No tuplet, duration,
note, or rest is synthesized from expected bar length.

The vendored sources are `src/org/audiveris/omr/sheet/symbol/DotFactory.java`
and `LedgerFragmentEvidence.java`. The reproducible upstream diff is
`0007-ledger-fragment-dot.patch`. Evidence and kill criteria are in
`docs/omr-rsi-cycle-16.md` and
`docs/omr-rsi-anh116-symbol-localization.md`. Numeral-overlap logic is not
included. Svc-22 requires a fresh host suite; the implementer does not run OMR.
