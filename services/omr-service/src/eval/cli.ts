...
import { createGeminiCallerFromEnv, hydrateGeminiKeyFromFiles, identifyAndLookup } from '../symbolic/visionId.js';
import { BENCH_SUITE, formatBench, runBench, writeBench } from './bench.js';
import { eraOfCorpusTitle, fromArtifacts, fromDocument, fromPdf, fromScoreFile, type Candidate } from './candidate.js';
...
import { compareShootout, fetchProdShootout, formatShootout, runLocalShootout, shootoutScorePath } from './shootout.js';
...
const main = async (): Promise<number> => {
    ...
    if (
        command === undefined ||
        (command !== 'help' && command !== 'bench' && command !== 'symbolic' && slug === undefined)
    ) {
        return fail(usage);
    }
    if (command === 'help') {
    ...
    switch (command) {
        ...
        case 'audiveris': {
            ...
            const candidate = await fromPdf(pdfPath, values['force-audiveris'] === true, eraOfCorpusTitle(entry.title));
            process.stdout.write(
        ...
        case 'run': {
            ...
            if (from === 'pdf') {
                if (entry.pdf.sha256 === undefined) {
                    return fail('pdf.sha256 is required before --from pdf; pin the file you actually scored');
                }
                const fetched = await fetchCorpus(entry, { allowNetwork: false, mode: 'all' });
                const pdfPath = fetched.pdfPath;
                if (pdfPath === null) {
                ...
                candidate = await fromPdf(pdfPath, values['force-audiveris'] === true, eraOfCorpusTitle(entry.title));
                ...
                candidate = await fromArtifacts(dir, 'artifacts', eraOfCorpusTitle(entry.title));
                midiDir = fetched.midiDir;
