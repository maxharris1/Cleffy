...
# First non-empty value of NAME from a dotenv file (quotes stripped).
boot_common_dotenv_value() {
...
# transcribe-ink reads GEMINI_API_KEY (then the Google names) from the edge
# env file. OMR vision already hydrates the same names from gitignored
# `.env` / `services/omr-service/.env` / `.env.local` and the process env
# (Cursor Secrets). Copy that path into functions/.env so local serve works.
hydrate_gemini_keys() {
    boot_common_init_repo
    local name file value
    if [ -n "${GEMINI_API_KEY:-}" ] || [ -n "${GOOGLE_GENERATIVE_AI_API_KEY:-}" ] || [ -n "${GOOGLE_API_KEY:-}" ]; then
        return 0
    fi
    for file in "$BOOT_COMMON_REPO_ROOT/.env" "$BOOT_COMMON_REPO_ROOT/services/omr-service/.env" "$BOOT_COMMON_REPO_ROOT/supabase/functions/.env" "$BOOT_COMMON_REPO_ROOT/.env.local"; do
        for name in GEMINI_API_KEY GOOGLE_GENERATIVE_AI_API_KEY GOOGLE_API_KEY; do
            if [ -z "${!name:-}" ]; then
                value="$(boot_common_dotenv_value "$file" "$name" || true)"
                if [ -n "$value" ]; then
                    export "${name}=${value}"
                fi
            fi
...
# Generated by the Cleffy boot scripts (local Supabase only) — do not edit.
# Optional secrets (ANTHROPIC_API_KEY, GEMINI_API_KEY, etc.) come from the
# caller's environment: the Cursor Secrets tab, the shell, or the same
# gitignored .env files OMR vision already reads.
